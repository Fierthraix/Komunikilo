...
def plot_multi_roc_curve(lrs: List[pd.DataFrame]):
    fig, ax = plt.subplots()
...
def plot_multi_youden_j_and_auc_vs_snr(
    labels: List[str], multi_lrs: List[List[pd.DataFrame]], snrs: List[float]
):
...
def __run(tx: Tx, snrs: List[float]):
    NUM_BITS: int = 120
    NUM_ATTEMPTS: int = 150

    # Calculate Eb
    print("entered __run function")
    signal: np.array = tx(random_data(NUM_BITS))
    print("got first signal")
    while len(signal) < 4096 + 64:
        NUM_BITS += 10
        signal: np.array = tx(random_data(NUM_BITS))
    print(f"got num_bits : {NUM_BITS}")
...
if __name__ == "__main__":
    ...
    snrs: np.array = undb(np.linspace(-100, -5, 15))
    ...
    # __run(bpsk_signal, snrs)
    # __run(cdma_signal, snrs)
    # __run(ofdm_signal, snrs)
    print("starting the sim")
    __run(fh_ofdm_dcsk_signal, snrs)
